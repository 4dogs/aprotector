 ...
 * 执行子模式，比如debugging，profiling等
 * 这些标志位主要被mterp中的汇编代码以及JIT生成的代码使用
 */
 ...
 * handler.
 * 这里定义一些解释器中断标记。当某个标记被设置的时候，解释器会中断当前的正常的解释流程
 * 然后会调用相关联的中断回调。
 */

enum InterpBreakFlags {
    kInterpNoBreak            = 0x00,    /* Don't check  不检查*/
    kInterpSingleStep         = 0x01,    /* Check between each inst  设置了单步，那么每条指令都要调用单步处理回调*/
    kInterpSafePoint          = 0x02,    /* Check at safe points 安全指针检查 */
};
...
#endif
    struct InterpSaveState* prev;  // To follow nested activations 考虑到一些嵌套的调用，所以可能在使用当前的状态的时候突然去解释另外一段去了
                                                  // 所以这里需要保存下前一个状态，但是这里只是个指针。
