    ...
    const ClassObject* clazz)
{
     ...
     */
    /*ÔÚDEXÎÄ¼þÖÐ²éÕÒclassË÷Òý,ÎªÁËÓÐ¸üºÃµÄÐÔÄÜ£¬ÎÒÃÇÓ¦¸Ã´¢´æÕâÔÚClassObject¶ÔÏóÖÐ*/
 ...
 */
/*
 * ¶ÁÈ¡Ò»¸öÓÐ·ûºÅµÄÕûÐÎ£¬'zwidth'ÊÇ´ÓÁã¿ªÊ¼µÄ×Ö½ÚÊý
 */
 ...
 */
/*
 * ¶ÁÈ¡Ò»¸öÎÞ·ûºÅµÄÕûÐÎ,'zwidth'ÊÇ´ÓÁã¿ªÊ¼µÄ×Ö½ÚÊý,'fillOnRight'±êÊ¾ÓÒ±ßÎÒÃÇÐèÒªÁãÀ´Ìî³ä
 */
 ...
 */
/*
 * ¶ÁÈ¡Ò»¸öÓÐ·ûºÅµÄ³¤ÕûÐÎ.'zwidth'ÊÇ´ÓÁã¿ªÊ¼µÄ×Ö½ÚÊý
 */
 ...
 */
/*
 * ¶ÁÈ¡Ò»¸öÎÞ·ûºÅµÄ³¤ÕûÐÎ,'zwidth'ÊÇ´ÓÁã¿ªÊ¼µÄ×Ö½ÚÊý,'fillOnRight'±êÊ¾ÓÒ±ßÐèÒªÎÒÃÇÓÃÁãÀ´Ìî³ä
 */
 ...
 */
/*
 * Í¨¹ýË÷Òý¹ØÁªµ½Ò»¸ö·½·¨.Õâ½ö½ö¸ø¶¨ÎÒÃÇÀàµÄÃû×ÖºÍ·½·¨Ãû³ÆºÍ·½·¨Ç©Ãû£¬ÎÒÃÇÐèÒªÈ¥ÕÒ·½·¨µÄÀà£¬
 * È»ºóÕÒÀàÀïÃæµÄ·½·¨.Èç¹ûÖ®Ç°·½·¨»¹Ã»ÓÐ±»½âÎö£¬ÎÒÃÇ¿ÉÒÔÊ¹ÓÃÒÔÇ°²éÑ¯µÄ½á¹û
 */
 ...
 * ×¢½â±»¶¨ÒåÔÚ'clazz'µÄÀàÖÐ£¬µ±'resultStyle'ÊÇ'kAllRaw'Ê±£¬Ëü¿ÉÄÜÊÇNULL
 *
 ...
 * Èç¹û'resultStyle'ÊÇ'kAllObjects'£¬½á¹û½«×ÜÊÇÒ»¸öºÏÊÊÀàÐÍµÄ¶ÔÏó.¶ÔÓÚ»ù±¾Êý¾Ý
 * ÀàÐÍ£¬Í¨³£ÒÔ°ü×°ºó¶ÔÏó½«±»´´½¨
 *
 ...
 * Èç¹û'resultStyle'ÊÇ'kAllRaw',ÄÇÃ´Êý×Ö³£Á¿½«Ö±½Ó±»±£´æµ½'pValue',²¢ÇÒ
 * ±»±àÈëË÷ÒýµÄÖµ(Èç:StringtºÍMethod)½«×÷ÎªË÷Òý±»·µ»Ø£¬¸´ÔÓµÄÖµ(Èç:annotations ºÍ arrays)
 * ²»È¥´¦Àí
 *
 ...
 * ²¢ÇÒÆäËûÒ»ÇÐ×÷ÎªºÏÊÊÀàÐÍ¶ÔÏó±»¹¹Ôì
 *
 * The caller must call dvmReleaseTrackedAlloc on returned objects, when
 * using "kAllObjects" or "kPrimitivesOrObjects".
 * µ±Ê¹ÓÃ'kAllObjects'»òÕß'kPrimitivesOrObjects'Ê±£¬µ÷ÓÃÕß±ØÐëµ÷ÓÃdvmReleaseTrackedAllocº¯Êý
 * ·µ»Ø¶ÔÏó
 *
 ...
 * "valueObj".
 * ¶ÔÓÚ´ó¶àÊýµÄ¶ÔÏóÀàÐÍ£¬ÎÒÃÇÔÚÕâ¶ùÃ»ÓÐ×öÌ«¶àµÄÊÂÇé£¬Ö»½ö½ö·µ»ØÁË'valueObj'
 * 
 ...
 * ¶ÔÓÚÒ»¸öÊý×é×¢½â, ÌáÈ¡µÄ¶ÔÏóµÄÀàÐÍ×ÜÊÇjava.lang.Object[]£¬µ«ÊÇ£¬
 * ÎÒÃÇÏëÒªËüµÄ×¢½â³ÉÔ±È¥·µ»ØÏàÆ¥ÅäÀàÐÍ£¬ÔÚÄ³ÖÖÇé¿öÏÂ£¬Õâ¿ÉÄÜÇ£Éæµ½²ðÏäµÄ»ù±¾Êý¾ÝÀàÐÍ
 *
 ...
 * annotation's class.
 * ×¢½â±»¶¨ÒåÔÚ'clazz'ÀàÖÐ. 'pPtr'Ö¸ÏòÒ»¸öÖ¸ÕëµÄ×¢½âÊý¾ÝÇø.'annoClass'ÊÇÒ»¸ö×¢½âÀà
 *
 * We extract the annotation's value, create a new AnnotationMember object,
 * and construct it.
 * ÎÒÃÇ³éÈ¡×¢½âÖµ£¬´´½¨Ò»¸öÐÂµÄ×¢½â³ÉÔ±¶ÔÏó²¢ÇÒ¹¹ÔìËü,'pPtr'Ö¸ÕëÖ¸ÏòÁË
 *
 ...
 */
/*
 * ÎÒÃÇ´Ó×¢½âÏîÖÐ´´½¨Ò»¸öÐÂµÄ×¢½â¶ÔÏó.
 *
 * ×¢½â±»¶¨ÒåÔÚ'clazz'ÀàÖÐ£¬'pPtr'Ö¸ÏòÁËÒ»¸öÖ¸ÕëµÄ×¢½âÊý¾ÝÇø
 * 
 ...
 * on alloc failure.
 * ·µ»ØÒ»¸ö×¢½âÊý×é¶ÔÏó£¬»òÕßÊÇNULL,·ÖÅäÊ§°Ü½«Òý·¢Òì³£
 */
 ...
 */
/*
 * ÔÚ×¢½â¼¯ºÏ('pAnnoSet')ÖÐ·µ»ØÒ»¸öÖ¸¶¨ÀàÐÍµÄ×¢½âÏî; Èç¹ûÈç¹û×¢½â¼¯ºÏÖÐ²»°üÀ¨Ö¸¶¨ÀàÐÍµÄ×¢½â£¬Ôò·µ»ØNULL
 */
 ...
 */
/*
 * ÔÚ×¢½â¼¯ºÏÖÐ·µ»ØÒ»¸öÖ¸¶¨ÀàÐÍµÄ×¢½â¶ÔÏó£¬Èç¹û¸Ã¼¯ºÏÃ»ÓÐ°üº­Ö¸¶¨ÀàÐÍµÄ×¢½â£¬Ôò·µ»ØNULL
 */
 ...
 */
/*
 * ±È½ÏDEXÎÄ¼þËùÌá¹©µÄÃèÊö·ûÖÐµÄÀàµÄÃû³Æ¡£
 */
 ...
 */
/*
 * ÔÚ×¢½âÏîÖÐ²éÕÒÒ»¸ö×¢½âÖµ£¬ÆäÃû³ÆÆ¥Åä'name'
 */
 ...
 */
/*
 * ´ÓÖ¸¶¨×Ö¶ÎÖµ'annoName'ÌáÈ¡Ò»¸ö×¢½â±àÂëÖµ(Õâ¸öÖµ¿ÉÄÜÊÇÊý×é¶ÔÏó£¬ÆÕÍ¨¶ÔÏó,Òì³£µÈ...)
 *
 * 'expectedType'ÊÇÒ»¸ö×¢½âÖµÀàÐÍ, ÀýÈç kDexAnnotationString.
 ...
 */
 /*
 * ²éÕÒÒ»¸öÇ©ÃûÊôÐÔ²¢³éÈ¡ËüµÄÖµ.(Ç©ÃûÄÜ±»ÕÒµ½ÔÚ×¢½âÀà£¬¹¹Ôìº¯Êý£¬·½·¨ºÍ×Ö¶ÎÖÐ)
 */
 ...
 */
/*
 * ·µ»ØÕâ¸öÀàµÄ×¢½âÊý×é¶ÔÏó.Èç¹ûÕâ¶ùÃ»ÓÐ×¢½âÖµ£¬Ôò·µ»Ø¿Õ
 */
 ...
 */
¡/*
 * ´Ó×¢½âÖÐµÃµ½EnclosingMethodÊôÐÔ.·µ»ØÒ»¸öMethod¶ÔÏó»òNULL
 */
 ...
 */
/*
 * ÕÒµ½Ò»¸ö·â±ÕÀà,·µ»ØEnclosingClassµÄÊôÐÔÖµ
 */
ClassObject* dvmGetDeclaringClass(const ClassObject* clazz)
{
     ...
     */
    /*
   * EnclosingClass×¢½âÓÐÒ»¸ö³ÉÔ±¡®Class value¡¯
   */
 ...
 */
 /*
 * ²éÕÒÒ»¸ö·â±ÕÀà£¬ÎÒÃÇÊ×ÏÈ²éÕÒÒ»¸öEnclosingClassÊôÐÔ,²¢ÇÒÈç¹ûÃ»ÓÐ±»ÕÒµ½£¬ÔòÎÒÃÇ²éÕÒÒ»¸öEnclosingMethod
 */
 ...
 */
/*
 * ´ÓÒ»¸ö×¢½âÖÐ»ñÈ¡EnclosingÊôÐÔ.Èç¹ûÕÒµ½ÁË£¬Ôò·µ»Øtrue.Í¨¹ý²ÎÊý ÀàµÄÔ­Ê¼Ãû³Æ×Ö·û´®ºÍ
 ...
 */
 /*
  * ´ÓMemberClasses×¢½âÖÐ³éÈ¡Class[]£¬MemberClasses×¢½âÓÐÒ»¸ö³ÉÔ±'Class[] value'
  * 
  */
 ...
 */
/*
 * Ö¸¶¨·½·¨µ½'method'±È½ÏÊôÐÔ£¨ÀàÃû£¬·½·¨Ãû£¬·½·¨Ç©Ãû£©
 */
 ...
 */
/*
 * ¸ù¾Ý'method'·µ»ØDexAnnotationSetItem
 */
 ...
 */
 /*
 * ¸ù¾Ý'method'·µ»Ø×¢½âÊý×é¶ÔÏó£¬Èç¹ûÕâ¶ùÃ»ÓÐ×¢½âÔò·µ»Ø¿Õ
 */
 ...
 */
/*
 * »ñÈ¡×¢½â¼¯'pAnnoSet'ÖÐkDexVisibilityRuntimeÀàÐÍµÄ×¢½â¶ÔÏó
 */
 ...
 */
/*
 * »ñÈ¡×¢½âÇ©Ãû£¬Èç¹ûÃ»ÓÐÇ©Ãû´æÔÚÔò·µ»ØNULL
 */
 ...
 */
/*
 * ´Ó'system'×¢½âÁÐ±íÖÐÌáÈ¡Òì³£Êý×é¶ÔÏó
 *
 * Throws×¢½âÓÐÒ»¸ö³ÉÔ±'Class[] value'
 */
 ...
 */
/*
 * Ö¸¶¨×Ö¶Î'field',±È½ÏÊôÐÔ(ÀàÃû£¬×Ö¶ÎÃû£¬×Ö¶ÎÇ©Ãû)
 */
 ...
 */
/*
 * ¸ù¾Ý'field'·µ»ØDexAnnotationSetItem
 */
 ...
 */
 /*
 * ¸ù¾Ý'field'·µ»Ø×¢½âÊý×é¶ÔÏó£¬Èç¹ûÕâ¶ùÃ»ÓÐ×¢½âÔò·µ»Ø¿Õ
 */
 ...
 */
/*
 * ÎÒÃÇÓÐÒ»¸öannotation_set_ref_list£¬Õâ±¾ÖÊÉÏÊÇÒ»¸öÌõÄ¿ÁÐ±í£¬
 * ÎÒÃÇ´«µÝ¸øprocessAnnotationSet()
 *
 * ·µ»ØµÄ¶ÔÏó±ØÐë±»ÊÍ·ÅdvmReleaseTrackedAlloc
 */
 ...
 */
/*
 * Í¨¹ý'method'²éÕÒ DexAnnotationSetItem
 */
 ...
 * ·µ»ØÒ»¸öµÄ×¢½â¶ÔÏóÊý×éµÄÊý×é,Íâ²¿Êý×éÓÐ·½·¨²ÎÊýÌõÄ¿£¬ÄÚ²¿Êý×éÓÐÓë¸Ã²ÎÊýÓÐ¹ØµÄ×¢ÊÍÁÐ±í
 *
 ...
 * Èç¹û·½·¨Ã»ÓÐ²ÎÊý,ÎÒÃÇ·µ»ØÒ»¸öÁã³¤¶ÈµÄÊý×é.Èç¹û¸Ã·½·¨ÓÐÒ»¸ö»òÕß¶à¸ö²ÎÊý,
 * ÄÇÃ´·µ»ØÒ»¸öÊý×é£¬Æä³¤¶ÈµÈÓÚ²ÎÊýµÄÊýÄ¿£¬Èç¹ûÒ»¸ö¸ø¶¨µÄ²ÎÊý²»¾ßÓÐÒ»¸ö×¢ÊÍ£¬
 * ÏàÓ¦µÄÌõÄ¿½«ÊÇ¿Õ¡£
 *
 * Caller must call dvmReleaseTrackedAlloc().
 */
 ...
 */
/*
 * ÒÆ¶¯ËüµÄË÷Òý.´Óµü´úÆ÷ÖÐ·µ»ØÏÂÒ»¸ö½âÃÜÖµ
 */
