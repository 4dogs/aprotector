 ...
 * 单个的原子缓存结构描述
 * 这个要保存两个key，因为比如遇到instanceof这样的操作符，他是二元操作符，有左操作数和右操作数
 * 
 */
 ...
 */
struct AtomicCache {
    AtomicCacheEntry*   entries;        /* array of entries 存放多个单个缓冲描述单元*/
    int         numEntries;             /* #of entries, must be power of 2 缓存单元的个数，必须是2的倍数*/

    void*       entryAlloc;             /* memory allocated for entries 为缓存单元分配的内存*/

    /* cache stats; note we don't guarantee atomic increments for these 以下是一些状态值*/
