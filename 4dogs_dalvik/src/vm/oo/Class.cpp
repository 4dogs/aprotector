 ...
 */
/*
 * Àà¼ÓÔØ£¬°üÀ¨Òýµ¼Àà¼ÓÔØÆ÷£¬Á´½ÓºÍ³õÊ¼»¯
 *
 */
 ...
 */
/*
 * ÀàÐòÁÐºÅÒÔ 0x50000000 ¿ªÊ¼.ÎÒÃÇÊ¹ÓÃÒ»¸ö·ÇÁã³õÊ¼Öµ,ÒÔ±ãËûÃÇÊ¹ÓÃ¶þ½øÖÆ×ª´¢Êä³ö
 *
 */
 ...
 */
/*
 * ÌØ¶¨µÄÊ±¼ä¶ÎÄÚ£¬¹ØÓÚÒ»Ð©Àà±»¼ÓÔØµÄÈÕÖ¾ÐÅÏ¢
 */
 ...
 * ºÏ³ÉÒ»Ð©Ô­ÉúÀà;
 * 
 * ½ö´´½¨ÀàºÍ·µ»ØËü(²»»á½«ËüÌí¼Óµ½ÀàÁÐ±íÖÐ)
 ...
 */
/*
 * ÊµÀý»¯Àà²¢³õÊ¼»¯Ëü.ÕâÐ©ÊÇÓÉClassÀàµÄÊµÀýºÍËùÓÐÔ­ÉúÀàÐÍËù´ú±íµÄÀà×é³É.
 * 
 * ¼ÓÔØÁË9´ó»ù±¾ÀàÐÍ
 */
static bool createInitialClasses() {
   ...
   */
    ¡/*
   * ³õÊ¼»¯ÁËClassÀà£¬Õâ¶ù²»µÃ²»È¥Íê³ÉÌØ±ðµÄ£¬ÒòÎªËüµÄÊµÀýÊÇËû±¾Éí
   */
   ...
   */
    /*
   * ³õÊ¼»¯Ô­ÉúÀàÐÍËù´ú±íµÄÀà(eg.PRIM_VOID = typeVoid),ÕâÐ©¶¼ÊÇClassµÄÊµÀý
   */
    bool ok = true;
 ...
 *
 * Õâ¸öº¯ÊýÓÃÀ´³õÊ¼»¯Æô¶¯Àà¼ÓÔØÆ÷£¨Bootstrap Class Loader£©£¬Í¬Ê±»¹»á³õÊ¼»¯java.lang.ClassÀà¡£
 * Æô¶¯Àà¼ÓÔØÆ÷ÊÇÓÃÀ´¼ÓÔØJavaºËÐÄÀàµÄ£¬ÓÃÀ´±£Ö¤°²È«ÐÔ£¬¼´±£Ö¤¼ÓÔØµÄJavaºËÐÄÀàÊÇºÏ·¨µÄ
 */
bool dvmClassStartup()
{
   ...
   /* ´´½¨hash±í*/
     ...
     */
    /*
   * ´¦Àí»ù±¾Àà¿â£¬Õâ¶ùµÄÒâË¼ÊÇÈ¥´ò¿ªÖ¸¶¨µÄDEX»òÕßJarÎÄ¼þ£¬²¢ÇÒ¾¡¿ÉÄÜÍ¨¹ýÓÅ»¯Æ÷È¥ÔËÐÐËûÃÇ
   *
   */
 ...
 */
/*
 * Dump 'ClassPathEntry'½á¹¹ÌåÊý×éµÄÄÚÈÝ
 *
 */
static void dumpClassPath(const ClassPathEntry* cpe)
{
    int idx = 0;
   /*ÅÐ¶ÏÈç¹û²»ÊÇ×îºóÒ»Ìõ£¬Ôò½øÈëÑ­»·½øÐÐÅÐ¶ÏÖÖÀà*/
 ...
 */
/* 
 * ÅÐ¶ÏÖ¸¶¨µÄÂ·¾¶ÊÇ·ñ±»°üº¬;Èç¹û'cpe'°üº¬ÁË'path'Ôò·µ»Øtrue,·´Ö®Îªfalse
 */
bool dvmClassPathContains(const ClassPathEntry* cpe, const char* path)
{
    while (cpe->kind != kCpeLastEntry) {
		// º¯ÊýÊÇ±È½ÏÁ½¸ö×Ö·û´®»òÕß±äÁ¿µÄ´óÐ¡,·µ»Ø±È½ÏµÄ½á¹û£¬'>'ÔòÎªÕýÊý,'='ÔòÎª0,'<'ÔòÎª¸ºÊý
 ...
 */
/*
 * ÊÍ·ÅClassPathEntry½á¹¹ÌåÊý×é,Ê×ÏÈÎÒÃÇÏÈÊÍ·ÅÃ¿¸öÌõÄ¿µÄÄÚÈÝ£¬È»ºóÔÙÊÍ·ÅËü±¾Éí
 */
static void freeCpeArray(ClassPathEntry* cpe)
{
 ...
 */
/*
 * »ñÈ¡ÎÄ¼þÃû³ÆµÄºó×º,Èç¹ûÃ»ÓÐÕÒºó×º£¬ÔòÎª'<none>',Èç¹ûÄÜÕÒµ½µÄ»°£¬Ôò¾ÍÊÇ×îºóÒ»¸ö'.'Ö®ºóµÄ¶«¶«.
 */
 ...
 */
/*
 * ×¼±¸Ò»¸öClassPathEntry½á¹¹Ìå£¬ÔÚÕâ¶ùÖ»ÄÜÓÐÒ»¸öÓÐÐ§µÄÎÄ¼þÃû£¬ÎÒÃÇÐèÒªÅªÇå³þËüÊÇÊ²Ã´ÑùµÄÀàÐÍÎÄ¼þ,
 * ²¢ÇÒ¶ÔÈÎºÎ²»Í¬ÀàÐÍÎÄ¼þ£¬ÎÒÃÇ¶¼ÐèÒª´ò¿ªËü²¢ÇÒ¿´¿´Àïµ½µ×ÊÇÊ²Ã´
 */
static bool prepareCpe(ClassPathEntry* cpe, bool isBootstrap)
{
    struct stat sb;
	/* stat ÊÇ½«ÎÄ¼þµÄÒ»Ð©ÐÅÏ¢¸³Öµ¸øsb½á¹¹±äÁ¿*/
 ...
 */
 /*
 * ×ª»»Ò»¸öÓÃÃ°ºÅ·Ö¸ô·ûµÄÄ¿Â¼ÁÐ±í(Zip,DEX)½øÈë'ClassPathEntry'½á¹¹ÌåÊý×é.
 *
 * ÔÚÕý³£Æô¶¯ÆÚ¼ä£¬Èç¹ûÕâ¶ùÃ»ÓÐÌõÄ¿´æÔÚ£¬½«»áÆô¶¯Ê§°Ü£¬ÒòÎªÔÚÈ±ÉÙ»ù±¾ÓïÑÔÖ§³ÖµÄÀàÏÂ£¬ÎÒÃÇÊÇÃ»·¨»ñÈ¡¸ü¶àµÄ£¬µ«ÊÇ£¬Èç¹ûÕâÊÇ¸öÓÅ»¯µÄDexÎÄ¼þ£¬ÎÒÃÇÊÇÔÊÐíËüµÄ
 *
 * Èç¹û´Ó»ù±¾Àà¿âÂ·¾¶ÖÐÌí¼Ó»òÒÆ³ýÒ»Ð©ÌõÄ¿,ÄÇÃ´ÔÚDexÎÄ¼þÖÐµÄÒÀÀµ¹ØÏµ½«»á±»ÆÆ»µ£¬²¢ÇÒ³ýµÚÒ»¸öÌõÄ¿Ö®ÍâµÄÒ»ÇÐ¶¼ÐèÒªÒªÖØÐÂÉú³É
 */
static ClassPathEntry* processClassPath(const char* pathStr, bool isBootstrap)
{
    /*
   *½¨Á¢½á¹¹Ìå ClassPathEntry À´´æ´¢Ò»¸öÂ·¾¶ÐÅÏ¢
  */
  ...
  */
   ...
   */
     /*
   * ÉèÖÃÒ»¸öÈ«¾ÖµÄÖ¸Õë£¬ÒÔ±ãDexÎÄ¼þËùÒÀÀµµÄ¶«Î÷ÄÜÕÒµ½Ëü
   */
     ...
     */
	   ...
	   /* ×¼±¸ÀàÂ·¾¶½Úµã,prepareCpe ÖÐµ÷ÓÃÁË(dvmJarFileOpen/...) ´ò¿ªÒ»¸ö jar °ü*/
...
bail:
    free(mangle);
    /*
   * ËùÓÐ±»¼ÓÔØµÄ Jar °üÀà¶¼·Åµ½È«¾Ö¶ÔÏñ gDvm.bootClassPath ÖÐ´æ´¢¡£
   * ¶ÔÏñµÄ½áÎ²¶¼ÊÇÓÃÒ»¸öÌØÊâµÄ½á¹¹ÌåÀ´±íÊ¾µÄ£¬ÕâÀïÊÇ kind = kCpeLastEntry ±íÊ¾ÊÇ×îºóÒ»¸öÀàÂ·¾¶½Úµã£¬ÒòÐ©²»ÓÃÁíÍâÒ»¸ö±äÁ¿±íÊ¾´óÐ¡
   */
 ...
 */
/*
 * ´Ó»ù±¾µÄÀà¿âÖÐ²éÕÒ£¬¼ÓÔØÒ»¸öÓëÃèÊö·ûÏàÆ¥ÅäµÄÀà;
 */
 ...
 */
/*
 * »ñÈ¡bootClassPathµÄ×ÜÊý
 */
int dvmGetBootPathSize()
{
    const ClassPathEntry* cpe = gDvm.bootClassPath;

    /*Èç¹ûÃ»ÓÐµ½×îºóÒ»Ìõ£¬ÔòÖ¸ÕëÏòÏÂÒÆ¶¯*/
    while (cpe->kind != kCpeLastEntry)
        cpe++;
 ...
 */
/*
 * ÅÐ¶ÏÕâ¸ö'loader'ÊÇ·ñ³öÏÖÔÚÀà¼ÓÔØÆ÷ÁÐ±íÖÐ
 * 
 */
 ...
 */
 /*
  * Èç¹û (loader != clazz->classLoader),ÄÇÃ´½«Ìî¼Ó'loader'µ½×î³õµÄÀà¼ÓÔØÆ÷ÁÐ±íÖÐ.
  * Ò»°ãÇé¿öÏÂ£¬ÕâÊÇÒ»¸ö¶ÌÁÐ±í£¬ÒòÐ©ÔÚÕâ¶ù²»ÐèÒª×öÌ«¶àµÄÊÂÇé
  * gDvm.loadedClassesÊÇÍ¬²½µÄ£¬ÒòÐ©ÔÚÕâ¶ùµ÷ÓÃµÄÊ±ºò²»»á³ÖÓÐËü
  */
void dvmAddInitiatingLoader(ClassObject* clazz, Object* loader)
{
    if (loader != clazz->classLoader) {
         ...
         */
        /*
     * ÁÐ±íµÄ´óÐ¡²»»á¼õÐ¡£¬ÒòÐ©ÎÒÃÇ½ö½ö±£³ÖÔªËØÔÚËüÀïÃæµÄ¼ÆÊý£¬²¢ÇÒµ±ÔËÐÐ½áÊøÖ®ºó£¬ÎÒÃÇ½«ÖØÐÂ·ÖÅä´óÐ¡
     * Ö¸Õë×î³õÊÇ¿ÕµÄ£¬ÒòÐ©µ±¸öÊýÎª0Ê±£¬ÎÒÃÇ½«È¥ÖØÐÂ·ÖÅäËü
     */
 ...
 */
/*
 * ½«¼ÓÔØµÄClass·ÅÈëhash±í(gDvm.loadedClasses)ÖÐ£¬¼Ó¿ìÏÂ´ÎµÄ²éÕÒ
 */
 ...
 */
/*
 * ´Óhash±í(gDvm.loadedClasses)ÖÐÒÆ³ýClass
 */
 ...
 */
/*
 * »ñÈ¡ClassË÷Òý,ÈçÎª»ù±¾Àà¿âÔòµ÷ÓÃsearchBootPathForClassº¯Êý£¬·ñÔòµ÷ÓÃdexFindClassÕâ¸öº¯Êý
 * Ê¹ÓÃÖ¸¶¨µÄÆô¶¯Àà¼ÓÔØÆ÷ºÍÃèÊö·ûÀ´²éÕÒÖ¸¶¨Ãû³ÆµÄclass
 * Èç¹ûÀàÃ»ÓÐ±»¼ÓÔØ¹ý£¬Ëü½«±»¼ÓÔØºÍ³õÊ¼»¯£¬ÓÐ±ØÒªµÄ»°£¬ËüµÄ¸¸ÀàÒ²½«±»¼ÓÔØ
 * Èç¹ûÀà»¹ÊÇÃ»ÓÐ±»ÕÒµ½£¬Ôò·µ»ØNULL
 */
 ...
 */
 /*
 * ´ÓÖ¸¶¨µÄDEXÎÄ¼þÖÐ¼ÓÔØÒ»¸öÀà£¬Ê¹ÓÃÀà¼ÓÔØÆ÷È¥³õÊ¼»¯Ò»¸öÀà¶ÔÏó
 */
 ...
 */
/*
 * Í¨¹ýÉ¨Ãè»ù±¾Àà¿â£¬Èç¹ûËüÉÐÎ´±»¼ÓÔØ,¸ù¾Ý'descriptor'²éÕÒÖ¸¶¨µÄÀà
 */
 ...
 */
/*
 * ¸ºÔð¼ÓÔØClass²¢Éú³ÉÏàÓ¦ClassObjectµÄº¯Êý
 */
static ClassObject* findClassNoInit(const char* descriptor, Object* loader,
    DvmDex* pDvmDex)
{
 ...
 */
/*
 * Êµ¼Ê¼ÓÔØ¹ý³ÌÔÚÕâ¸öµØ·½£¬Ëü»áÎªClassObjectÊý¾Ý½á¹¹·ÖÅäÄÚ´æ£¬²¢ÇÒ¶ÁÈ¡DexÎÄ¼þµÄÏà¹ØÐÅÏ¢
 */
static ClassObject* loadClassFromDex(DvmDex* pDvmDex,
 ...
 */
/*
 * ÊÍ·ÅÏµÍ³¶ÑÖÐ·ÖÅäµÄClassObjectÈÎºÎ×ÊÔ´,ClassObject±¾Éí±»·ÖÅäÔÚGC¶ÑÖÐ
 */
 ...
 */
/*
 * ÊÍ·ÅÏµÍ³¶ÑÖÐ·ÖÅäµÄMethodÈÎºÎ×ÊÔ´
 */
static void freeMethodInnards(Method* meth)
{
#if 0
 ...
 */
/*
 * ¸´ÖÆÒ»¸öMethod½á¹¹Ìå,È·±£ÐÂ¿½±´µÄÈÎºÎ¶«Î÷ÄÜ±»ÊÍ·Å³öÈ¥
 */
 ...
 */
/*
 * ²éÕÒ×Ô¼ºµÄ¸¸Àà£¬Èç¸¸ÀàÃ»ÓÐ¼ÓÔØ£¬Ôò»á¼ÓÔØ¸¸Àà.ÈçÓÐinterface£¬Ôò¼ÓÔØÏàÓ¦µÄinterfaceÀà
 */
 ...
 */
/*
 * ³õÊ¼»¯Ò»Ð©¾²Ì¬×Ö¶Î£¬½«ËüÃÇµÄÖµ±£´æÔÚDEXÎÄ¼þÖÐ£¬Õâ±ØÐëÔÚÀà³õÊ¼»¯Ö®Ç°Íê³É
 */
 ...
 * Èç¹ûÒ»¸öÀàÃ»ÓÐ±»³õÊ¼»¯£¬È¥Ö´ÐÐ<clinit>´úÂë.Ëü±»ÃèÊöÔÚÐéÄâ»ú¹æ·¶v2.2.17.5
 *
 ...
 * monitor lock.
 * ¶à¸öÏß³Ì¿ÉÄÜÍ¬Ê±µ½´ïÕâ¶ù,Òò´Ëµ±ÎÒÃÇÈ¥¼ì²éµÄÊ±ºò,ÎÒÃÇÐèÒªÈ¥Ëø¶¨Àà
 * 
...
bool dvmInitClass(ClassObject* clazz)
{
   ...
   */
     /*
   * Èç¹ûµ±Ç°µÄÀà»¹Ã»ÓÐÍê³ÉÐ£Ñé£¬ÔòÖ´ÐÐÏÂÃæÕâ¶Î
   */
     ...
     */
       /*
    * ÎÒÃÇ²»Ö§³ÖÈ¥ÓÅ»¯Ò»¸öÎ´Ð£ÑéµÄÀà£¬µ«ÊÇÔÚ¿ª·¢ÆÚ¼ä£¬Õâ¸öÄ£Ê½ÊÇ¿ÉÄÜÊ¹ÓÃµÄ.
    * ÎÒÃÇ²»ÄÜÐ£ÑéÒ»¸öÒÑ¾­ÓÅ»¯¹ýµÄÀà£¬ÒòÎªÓÅ»¯¹ý³Ì»á¶ªÆúÒ»Ð©ÐÅÏ¢
    */
 ...
 */
/*
 * Ê¹ÓÃÐÂµÄÖµÈ¥Ìæ»»method->nativeFuncºÍmethod-insns. ±¾µØ·½·¨±»³É¹¦½âÎöÖ®ºó£¬Õâ½«Õý³£Ö´ÐÐ
 * eg. method->insns=insns; 
 *     method->nativeFunc=func;
 *
 * ²ÎÊýmethod±íÊ¾Òª×¢²áJNI·½·¨µÄJavaÀà³ÉÔ±º¯Êý£¬²ÎÊýfunc±íÊ¾JNI·½·¨µÄBridgeº¯Êý£¬²ÎÊýinsns±íÊ¾Òª×¢²áµÄJNI·½·¨µÄº¯ÊýµØÖ·¡£
 * µ±²ÎÊýinsnsµÄÖµ²»µÈÓÚNULLµÄÊ±ºò£¬º¯ÊýdvmSetNativeFunc¾Í·Ö±ð½«²ÎÊýinsnsºÍfuncµÄÖµ·Ö±ð±£´æÔÚ²ÎÊýmethodËùÖ¸ÏòµÄÒ»¸öMethod¶ÔÏóµÄ³ÉÔ±±äÁ¿insnsºÍnativeFuncÖÐ
 * ¶øµ±insnsµÄÖµµÈÓÚNULLµÄÊ±ºò£¬º¯ÊýdvmSetNativeFunc¾ÍÖ»½«²ÎÊýfuncµÄÖµ±£´æÔÚ²ÎÊýmethodËùÖ¸ÏòµÄÒ»¸öMethod¶ÔÏó³ÉÔ±±äÁ¿nativeFuncÖÐ
 */
 ...
 *'pMap'ÊÇÔÚ¶ÑÖÐ´æ·ÅµÄ£¬Èç¹ûMethod±»Ïú»Ù£¬ÄÇÃ´ËüÒ²½«±»ÊÍ·Å
 */
 ...
 */
/*
 * dvmFindLoadedClassº¯ÊýµÄ»Øµ÷º¯Êý
 */
 ...
 */
/*
 * dumpµ¥¸öÀà£¬±»dvmDumpClassµÈº¯Êýµ÷ÓÃ
 */
 ...
 */
/*
 * ±È½ÏÁ½¸ö·½·¨µÄÃû³ÆºÍÔ­ÐÍ,Ãû³Æ±È½ÏÒªÓÅÏÈÓÚÔ­ÐÍ±È½Ï;Èç¹ûÒª½øÐÐÔ­ÐÍ±È½Ï£¬ÔòÍ¨¹ýdvmCompareMethodProtos()º¯Êý
 */
 ...
 */
/*
 * ±È½ÏÁ½¸ö·½·¨Ãû³ÆºÍÔ­ÐÍ ,Ãû³Æ±È½ÏÒªÓÅÏÈÓÚÔ­ÐÍ±È½Ï;Èç¹ûÒª½øÐÐÔ­ÐÍ±È½Ï,ÔòÍ¨¹ýdvmCompareMethodParameterProtosº¯Êý
 */
 ...
 * Ãû³Æ±È½ÏÒªÓÅÏÈÓÚÔ­ÐÍ±È½Ï;Èç¹ûÒª½øÐÐÔ­ÐÍ±È½Ï,ÔòÍ¨¹ýdexProtoCompareº¯Êý
 */
 ...
 * Ãû³Æ±È½ÏÒªÓÅÏÈÓÚÔ­ÐÍ±È½Ï;Èç¹ûÒª½øÐÐÃèÊö·ûºÍÔ­ÐÍµÄ±È½Ï,ÔòÍ¨¹ýdvmCompareDescriptorAndMethodProto()º¯Êý
 */
